...
import { courseService, getUserRole } from '../utils/api';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // For now, we'll use mock data for the user name
        const mockUserData = { name: "John Doe", role: role };
        setUserData(mockUserData);

        // Fetch all available courses
        const coursesResponse = await courseService.getCourses();
        setCourseData(coursesResponse);

        // Mock data for attendance and results
        setAttendanceData([
